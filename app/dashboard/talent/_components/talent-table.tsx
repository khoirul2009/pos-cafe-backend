...
const fetchBanners = async ({ page }: { page: number }) => {
  const res = await axios.get(`/api/talent?page=${page}`);
  return res.data ?? [];
};
...
export default function TalentTable() {
  ...
  async function handleDelete(event: MouseEvent<HTMLButtonElement>) {
    try {
      const response = await axios.delete(`/api/talent/${deleteId}`);
      ...
      toast({
        title: 'Error',
        description: 'Failed to store Talent',
        variant: 'error'
      });
    }
  }
      ...
      cell: ({ row }) => (
        <div className="flex space-x-2">
          <Button variant="secondary" size="sm">
            <Link
              href={`/dashboard/talent/${row.original.id}/edit`}
              className="w-full"
            >
              Edit
            </Link>
