...
function fi(e) {
  let t = e.e,
    ...
    i = `Please refer to the documentation about Prisma's system requirements: ${ui(
...
function ds(e) {
  if (xc(e)) {
    ...
    return {
      dotenvResult: ps(t),
      message: ke(
        `Environment variables loaded from ${gt.default.relative(
          process.cwd(),
          e
        )}`
      ),
    ...
    async withRetry(t) {
      for (let r = 0; ; r++) {
        ...
        try {
          ...
          this.logEmitter.emit('warn', {
            message: `Attempt ${r + 1}/${wl} failed for ${t.actionGerund}: ${
              i.message ?? '(unknown)'
            }`,
            timestamp: new Date(),
            target: ''
  ...
  async getCurrentBinaryTarget() {
    {
      ...
      if (!Rl.includes(t))
        throw new T(
          `Unknown ${de('PRISMA_QUERY_ENGINE_LIBRARY')} ${de(
            H(t)
          )}. Possible binaryTargets: ${Ve(
            Rl.join(', ')
      ...
      if (e) {
        ...
        for (let [r, n] of Object.entries(e)) {
          ...
          if (typeof n != 'object' || Array.isArray(n))
            throw new N(`Invalid value ${JSON.stringify(
              e
